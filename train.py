...
class RetainGraphHook:
    def __init__(self):
        pass

    def __enter__(self):
        self.prev_hook = torch.Tensor.backward_hook
        torch.Tensor.backward_hook = self.backward_hook

    def __exit__(self, exc_type, exc_value, traceback):
        torch.Tensor.backward_hook = self.prev_hook

    def backward_hook(self, tensor):
        def hook(*args, **kwargs):
            kwargs['retain_graph'] = True
            return tensor._autograd_backward(*args, **kwargs)
        return hook
...
class SequenceLightningModule(pl.LightningModule):
    ...
    def poisson_encoder(self, x):
        encoded_img = self.PoissonEncoder(x)
        for t in range(self.hparams.model.d_model-1):
            encoded_img_temp = self.PoissonEncoder(x)
            encoded_img = torch.cat([encoded_img, encoded_img_temp], dim=-1)

        #print('Poisson Encoded Image: ', encoded_img.shape)
    ...
    def forward(self, batch):
        ...
        w = {}
        x_in, w = self.encoder(x, **z) # w can model-specific constructions such as key_padding_mask for transformers or state for RNNs

        x_out = torch.zeros_like(x_in)
        for i in range(1):
            x, state = self.model(x_in, **w, state=self._state)
            x_out += x
        x = x_out/1.
...
def create_trainer(config):
    ...
    print(config.trainer.enable_model_summary)
    trainer = pl.Trainer(
        logger=logger,
        callbacks=callbacks,
        profiler=profiler,
...
def train(config):
    ...
    if config.train.get("pretrained_model_path", None) is not None:
        ...
        model = SequenceLightningModule.load_from_checkpoint(
            ...
            config=config,
            strict=False #config.train.pretrained_model_strict_load,
        )
